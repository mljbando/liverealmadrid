                ...
                try{
                    ...
                    if( !($user_id = $this->get_meta_owner($this->umeta_id, $facebook_id)) ){
                        ...
                        if( !isset($profile['email']) || !is_email($profile['email'])){
                            $profile['email'] = $facebook_id."@".$this->pseudo_domain;
//                            $profile['email']='dummy@mlj-inc.com';
//                            throw new \Exception($this->mail_fail_string());
                        ...
                        update_user_meta($user_id, '_uniqid', crypt($user_id,sanitize_user($user_name)));
                        
                        $wpdb->update(
                            $wpdb->users,
                            array(
//                                'display_name' => $profile['name'],
                                'display_name' => $default_nickname,
...
//                        $this->welcome($profile['name']);
            ...
            case 'connect': // Connect user account to Facebook
                try{
                    ...
                    // Check email
                    if( !isset($profile['email']) || !is_email($profile['email']) ){// Make pseudo mail
                        $profile['email'] = $fb_uid."@".$this->pseudo_domain;
//                        $profile['email']='dummy@mlj-inc.com';
//                        throw new \Exception($this->mail_fail_string());
                    }
