...
//            $redirect_url = $this->get('redirect_to') ?: admin_url("profile.php");
            $redirect_url = $this->get('redirect_to') ?: get_bloginfo('url') . get_permalink();
            // Is user logged in?
            if( !is_user_logged_in() ){
                throw new \Exception($this->_('You must be logged in.'));
            }
...
EOS;
    ...
    public function login_form($is_register = false){
		$host = (isset($_SERVER['HTTP_WP_HOST']))? $_SERVER['HTTP_WP_HOST'] : $_SERVER['HTTP_HOST'];
		$redirect_url = $this->get('redirect_to') ?: 'http://'.$host.$_SERVER['REQUEST_URI'];
        echo $this->login_button($redirect_url, $is_register);
    }
