...
function create_post_related( $content = '' ){
		...
		if ( ( is_object($the_query_yuzo) && $the_query_yuzo->have_posts() && $wp_query->post_count != 0) || $counter_manual_post >= $metabox_add_post_first ) {
			...
			while ( (is_object($the_query_yuzo) && $the_query_yuzo->have_posts()) || $counter_manual_post >= $metabox_add_post_first ) :
				...
				if( $yuzo_options->style == 1 ){
					...
					//var_dump($yuzo_options->default_image);
					include (COMMONTEMPLATEPATH . '/insertpic_post_src.php');
					$image = $this->IF_get_image(  $yuzo_options->thumbnail_size,  $insertpic_src, /*$yuzo_options->default_image,*/ get_the_ID(), $order_image  );
					//$image = $this->IF_get_image(  $yuzo_options->thumbnail_size,  include (COMMONTEMPLATEPATH . '/insertpic_post_src.php'), /*$yuzo_options->default_image,*/ get_the_ID(), $order_image  );
						  $_html .= '
	<div class="relatedthumb relatedpost-'.get_the_ID().' '.$class_box_shadow.'" style="width:'.$width.'px;float:left;overflow:hidden;'.$css_title_center.'">  
		<a '.$rel_link.' href="'.get_permalink().'" '.$target_link.' >
			<div class="yuzo-img-wrap '.$css_shine_effect1.'" style="/*width: '.$width.'px;height:'.$height.'px;*/">'.$css_shine_effect2.'
			...
			</div>'.$my_array_views['top'].'
			<span class="yuzo__text--title" style="color: #00489b;font-size:'.$yuzo_options->font_size.'px;'.$bold_title.'">'.$if_utils->IF_setHtml( $if_utils->IF_cut_text( get_the_time('Y.m.d'), $yuzo_options->text_length , true ) ).'</span><br />
			<span class="yuzo__text--title" style="font-size:'.$yuzo_options->font_size.'px;'.$bold_title.'">'.$if_utils->IF_setHtml( $if_utils->IF_cut_text( get_the_title(), $yuzo_options->text_length , true ) ).'</span>'.$text2_extract .''.$my_array_views['bottom'].'
		</a>
	</div>';
					  		$style="<style>
