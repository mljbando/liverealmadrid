...
if (!class_exists("nxs_class_SNAP_TW")) { class nxs_class_SNAP_TW {
    ...
    function doPostToNT($options, $message){ global $nxs_urlLen; $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>''); 
      ...
      if ($options['attchImg']!=false && $img!='') $twLim = 116; else $twLim = 140; 
      
      require_once ('apis/tmhOAuth.php'); if ($nxs_urlLen>0) { $msg = nsTrnc($msg, $twLim-24+$nxs_urlLen); } else $msg = nsTrnc($msg, $twLim); //prr($msg); die('TTWWW');
