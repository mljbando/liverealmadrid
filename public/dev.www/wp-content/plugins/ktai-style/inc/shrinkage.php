...
//    const DIRECT_LINK_IMAGE_SIZE = 153600; // use ktai_direct_link_image_size filter to change this value
    const MAX_PAGE_NUM = 1000;
    const START_PAGING_REGEX = '@<!--start paging(\[(.*?)\])?-->\n*@';
    const END_PAGING_REGEX = '@\n*<!--end paging(\[(.*?)\])?-->@';
    const PAGE_NUMBER_PARAM = 'kp';
    static public $mobile_same_url = array( // Use same URL for PC and mobile
        'http://[-\w]+\.blog\d+\.fc2\.com/', 'http://(jugem|yaplog)\.jp/[-\w]+/', 'http://[-\w]+\.seesaa\.net/', 'http://blog\.goo\.ne\.jp/[-\w]+/', 'http://blogs\.dion\.ne\.jp/[-\w]+/', 'http://blog\.auone\.jp/[-\w]+/', 'http://[-\w]+\.blog\.so-net\.ne\.jp/', 'http://[-\w]+\.paslog\.jp/', 'http://[-\w]+\.vox\.com/', 'http://hb\.afl\.rakuten\.co\.jp/hgc/', 'http://[-\w]+\.ap\.teacup\.com/', 'http://pub\.ne\.jp/[-.\w]+/', 'http://[-\w]+\.blog.shinobi.jp/', // Redirect mobile URL from PC
        'http://d\.hatena\.ne\.jp/[-\w]+/', 'http://blog\.livedoor\.jp/[-\w]+/', 'http://[-\w]+.(cocolog|air|moe|tea|txt|way)-nifty\.com/', 'http://[-\w]+\.at\.webry\.info/', 'http://ameblo\.jp/[-\w]+/', 'http://[-\w]+\.spaces\.live\.com/', 'http://plaza\.rakuten\.co\.jp/[-\w]+(/|$)', 'http://[-\w]+\.blog.drecom.jp/', );
    static public $none_mobile_url = array( 'http://(www|support|app)\.cocolog-nifty\.com/', );
    ...
    public function __construct(){
        ...
        $this->url_host = (isset($url_parts['scheme'])?$url_parts['scheme']:'') . '://' . (isset($url_parts['host'])?$url_parts['host']:'') . ( isset($url_parts['port']) ? ':' . $url_parts['port'] : '' );
        ...
        $this->plugin_url_regex = '!^(' . preg_quote( $this->base->get( 'plugin_url' ), '!' ) . '|' . preg_quote( $this->base->strip_host( $this->base->get( 'plugin_url' ) ), '!' ) . ')!';
        ...
        $this->mobile_regex = '!^(' . implode( '|', apply_filters( 'mobile_same_url/ktai_style.php', apply_filters( 'ktai_mobile_same_url', self::$mobile_same_url ) ) ) . ')!';
        $this->none_mobile_regex = '!^(' . implode( '|', apply_filters( 'none_mobile_url/ktai_style.php', apply_filters( 'ktai_none_mobile_url', self::$none_mobile_url ) ) ) . ')!';
        $this->num_image = 0;
        $this->image_services = apply_filters( 'ktai_image_services', array( '^https?://[-\w.]+\.flickr.com/' => array( $this, 'get_thumbnail_flickr' ), ) );
        ...
        add_filter( 'post_thumbnail_html', array( $this, 'shrink_content' ), 90 );
 ...
 */
    private function make_url_regex( $full_url, $url_path ){
        $regex = '!^(' . preg_quote( $full_url, '!' ) // assume trailing shash
            . '?|' . preg_quote( $url_path, '!' ) . ( strlen( $url_path ) <= 1 ? ')!' : '?)!' );
 ...
 */
    private function set_image_inline(){
        ...
        if( $image_format && in_array( $image_format, array( 'inline', 'link' ) ) ){
            if( isset( $_COOKIE[self::COOKIE_IMAGE_INLINE] ) && ( $_COOKIE[self::COOKIE_IMAGE_INLINE] == 'inline' ) == $this->image_inline_default ){
 ...
 */
    public function complete_url( $redirect_url, $requested_url ){
        if( preg_match( '!^://!', $redirect_url ) ){
            ...
            $redirect_url = (isset($request['scheme'])?$request['scheme']:'') . $redirect_url;
 ...
 */
    public function convert_links( $content ){
        for( $offset = 0, $replace = 'X'; preg_match( '!<a ([^>]*?)>(.*?)</a>!s', $content, $a, PREG_OFFSET_CAPTURE, $offset ); $offset += strlen( $replace ) ){
            $orig = $a[0][0];
            $offset = $a[0][1];
            $attr_str = $a[1][0];
            $anchor = $a[2][0];
 ...
 */
    private function image_inside_link( $orig, $href, $url, $link_attr, $anchor, $src, $img_attr ){
        ...
        if( $this->image_inline && preg_match( $this->image_services_regex, $url ) && preg_match( $this->image_services_regex, $src ) ){
 ...
 */
    public function convert_images( $content ){
        for( $offset = 0, $replace = 'X'; preg_match( '!<img ([^>]*?)\bsrc=([\'"])(' . KtaiStyle::QUOTED_STRING_REGEX . ')\\2([^>]*?) ?/?>!s', $content, $img, PREG_OFFSET_CAPTURE, $offset ); // <?php /* syntax hilighting fix */
    ...
    private function image_to_link( $html, $src, $attr_str ){
        ...
        if( isset( $attr['class'] ) && preg_match( $this->keep_image_regex, $attr['class'] ) ){
            ...
            if( defined( 'KTAI_SHRINKAGE_DEBUG' ) && KTAI_SHRINKAGE_DEBUG && is_ks_error( $thumb_path ) ){
                ...
                $replace = $this->image_element( $thumb_url, $alt, $attr ) . ( preg_match( '/has_orig="true"/', $attr_str ) ? '[' : '' );
                $has_image = TRUE;
            } else{
 ...
 */
    private function link_element( $href, $anchor, $attr ){
        $html = sprintf( '<a href="%s"%s%s>%s</a>', $href, ( isset( $attr['class'] ) ? ' class="' . $attr['class'] . '"' : '' ), ( isset( $attr['style'] ) ? ' style="' . $attr['style'] . '"' : '' ), $anchor );
 ...
 */
    private function image_element( $src, $alt, $attr ){
        $html = sprintf( '<img src="%s"%s%s%s%s />', $src, ( isset( $alt ) ? ' alt="' . $alt . '"' : '' ), ( isset( $attr['class'] ) ? ' class="' . $attr['class'] . '"' : '' ), ( isset( $attr['style'] ) ? ' style="' . $attr['style'] . '"' : '' ), ( isset( $attr['align'] ) ? ' align="' . $attr['align'] . '"' : '' ) );
 ...
 */
    public function split_page( $buffer, $page_num ){
        ...
        if( strlen( $fragment ) < $buffer_length && isset( $added_html[1] ) ){
            ...
            return $header . $sep['start']['before'] . $navi . $sep['start']['after'] . $complemented . $sep['end']['before'] . $navi . $sep['end']['after'] . $footer;
        } else{
 ...
 */
    private function separate_buffer( $buffer ){
		$before = array();
		$after = array();
        $sep = array( 'start' => array( 'before' => '', 'after' => '<hr />' ), 'end' => array( 'before' => '<hr />', 'after' => '' ), );
 ...
 */
    public function trim_images( $content ){
        if( $this->base->get( 'cache_size' ) > 0 ){
            ...
            for( $offset = 0, $replace = 'X'; preg_match( '!<img ([^>]*?)\bsrc=([\'"])(' . KtaiStyle::QUOTED_STRING_REGEX . ')\\2([^>]*?) ?/?>!s', $content, $i, PREG_OFFSET_CAPTURE, $offset ); //" <?php /*syntax hilighting fix */
                 $offset += strlen( $replace ) ){
                $offset = $i[0][1];
                $replace = $i[0][0];
                $src = $i[3][0];
 ...
 */
    public function shrink_dropdown( $output ){
        $output = preg_replace( array( '/ class=([\'"])[-_ \w]+\1/', '/[\r\n\t]/' ), array( '', '' ), $output );
 ...
 */
    public function shrink_tag_cloud( $content, $tags, $args ){
        for( $offset = 0, $replace = 'X'; preg_match( '!<a href=([\'"])(' . KtaiStyle::QUOTED_STRING_REGEX . ')\\1([^>]*?)>(.*?)</a>!s', $content, $l, PREG_OFFSET_CAPTURE, $offset ); $offset += strlen( $replace ) ){
            $orig = $l[0][0];
            $offset = $l[0][1];
            $q = $l[1][0];
            $url = $l[2][0];
 ...
 */
    public function img_caption( $null, $attr, $content = NULL ){
        extract( shortcode_atts( array( 'id' => '', 'align' => 'alignnone', 'width' => '', 'caption' => '', 'margin' => 2, ), $attr ) );
