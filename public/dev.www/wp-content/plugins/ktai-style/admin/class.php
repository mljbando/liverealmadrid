...
public function set_auth_cookie($user_id, $remember = false, $secure = false) {
	$sid = false;
	if ( $user_id ) {
...
public function nonce_ays($action) {
	...
	if ( $this->get_referer() ) {
		$adminurl = esc_url($this->get_referer());
	} else {
	...
	if ($_POST) {
		...
		$html .= '<form method="post" action="' . esc_url($pagenow) . '"><input type="hidden" name="' . self::SESSION_NAME . '" value="' . esc_attr($this->sid) . '" />';
		...
		$add_html = '<p>' . esc_html($desc) . '<br /><a href="' . $adminurl . '">' . $no. '</a>' . ($allow_proceed ? ' <a href="' . esc_url(add_query_arg('_wpnonce', wp_create_nonce($action), $_SERVER['REQUEST_URI'])) . '">' . $yes . '</a>' : '') . ' </p>';
	}
