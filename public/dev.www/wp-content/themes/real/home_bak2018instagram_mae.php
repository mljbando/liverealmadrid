...
<?php
	// post_typeがnewsでカテゴリーがnewsの記事を8件出力する
	$news = new WP_Query(
		array( 'post_type' => 'bernabeu'
			, 'posts_per_page' => 3
			,'no_found_rows' => true
			,'update_post_meta_cache' => false)
		);
...
<?php if ( has_post_thumbnail() ): // サムネイルを持っているときの処理
	...
	if ( $image ) {
	  list($src, $width, $height) = $image;
		if($src){
			echo '<img src="'. esc_attr( $src ) .'" alt="'. ベルナベウLIVE！ .'" title="'. get_the_title() .'">';
		}
	}
?>
...
<?php endif; ?>			</p>
						<p class="berlive-data"><?php the_time('Y.m.d H:i'); ?></p>
						<p class="berlive-text"><?php the_title(); ?></p>
						<!--<p class="berlive-text"><?php the_content(); ?></p>-->
						</li>
<?php	// newsloop end.
	endwhile;
	// 投稿データをリセット
...
?>
								...
								<div id="nextarrowout">
								<p class="arrow"><a href="/bernabeu">一覧へ<i class="fa fa-angle-right fa-lg"></i></a></p>
								</div>

								<!--SP500-->
								<div id="SPnextarrowout">
								<p class="arrow"><a href="/bernabeu">一覧へ<i class="fa fa-angle-right fa-lg"></i></a></p>
								</div>
